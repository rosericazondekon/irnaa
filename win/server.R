...
server = function(input, output, session) {
  ...
  observe({
    ...
    norm_DESeq_data <- function(){
      ...
      counts.sf_normalized  <<- DESeq2::counts(dds_norm, normalized = TRUE)
      setProgress(value = 3/5, detail = "Count log-normalization...")
      log.norm.counts  <<- log2(counts.sf_normalized + 1)
      # DESeq.rlog  <<- rlog(dds_norm , blind = TRUE)
      if(dim(dds_norm)<1000){
        DESeq.rlog  <<- DESeq2::varianceStabilizingTransformation(dds_norm, blind = F) # added line for TCGA data
      }else{
        # DESeq.rlog  <<- DESeq2::varianceStabilizingTransformation(dds_norm, blind = F)
        DESeq.rlog  <<- DESeq2::vst(dds_norm) # added line for TCGA data
      }
    ...
    observeEvent(input$d_heatmap,{
      ...
      output$DESeq2_heatmap <- renderPlotly({
        withProgress(message = "Generating heatmap...Please wait!", value = 0.8,{
          heatmaply::heatmaply(as.matrix(hm.mat_DGEgenes), xlab = "Samples", ylab = "Features", 
    ...
    observeEvent(input$gprofile_btn,{
      withProgress(message = "Gene Pathway analysis in progress...",{
        ...
        gprof_table <<- as.data.frame(gProfileR::gprofiler(gprofile_genes
    ...
    observeEvent(input$l_heatmap,{
      ...
      output$limma_heatmap <- renderPlotly({
        withProgress(message = "Generating heatmap...Please wait!", value = 0.8,{
          heatmaply::heatmaply(as.matrix(hm.mat_DGEgenes.lima), xlab = "Samples", ylab = "Features", 
    ...
    observeEvent(input$limma_gprofile_btn,{
      withProgress(message = "Gene Pathway analysis in progress...",{
        ...
        setProgress(value = 2/6, detail = "gProfiler pathway Analysis...")
        limma_gprof_table <<- as.data.frame(gProfileR::gprofiler(limma_gprofile_genes
    ...
    observeEvent(input$edgeR_runDGE_btn,{
      withProgress(message = "Performing DGE Analysis with edgeR... Please wait!",{
        ...
        edgeR.DGElist  <- edgeR::estimateDisp(edgeR.DGElist , design)
        ...
        if(input$edgeR_model=="glmFit"){
          ...
          edger_fit <- edgeR::glmQLFit(edgeR.DGElist, design)
          setProgress(value = 8/n, detail = "Conducting empirical Bayes quasi-likelihood F-tests...")
          edger_qlf <- edgeR::glmQLFTest(edger_fit,coef=2)
          setProgress(value = 9/n, detail = "Extracting DGE results...")
          DGE.results_edgeR  <<- as.data.frame(edgeR::topTags(edger_qlf , n = Inf , # to  retrieve  all  genes
                                                       sort.by = "PValue", adjust.method = "BH"))
    ...
    observeEvent(input$e_heatmap,{
      ...
      output$edgeR_heatmap <- renderPlotly({
        withProgress(message = "Generating heatmap...Please wait!", value = 0.8,{
          heatmaply::heatmaply(as.matrix(hm.mat_DGEgenes.edgeR), xlab = "Samples", ylab = "Features", 
    ...
    observeEvent(input$edgeR_gprofile_btn,{
      withProgress(message = "Gene Pathway analysis in progress...",{
        ...
        setProgress(value = 2/6, detail = "gProfiler pathway Analysis...")
        edgeR_gprof_table <<- as.data.frame(gProfileR::gprofiler(edgeR_gprofile_genes
